...
// Game objects
const ballRadius = 8; // Original size
const paddleHeight = 15;
const paddleWidth = 100; // Original size
const brickRowCount = 5;
const brickColumnCount = 10;
const brickWidth = (canvas.width - 50) / brickColumnCount; // Adjusted for new size
...
let ball = {
    x: canvas.width / 2, // Centered in the new canvas size
    y: canvas.height - 30, // Positioned above the bottom
    dx: 4,
    dy: -4
};

let paddle = {
    x: (canvas.width - paddleWidth) / 2, // Centered in the new canvas size
    y: canvas.height - paddleHeight - 10 // Positioned above the bottom
};
...
function drawPaddle() {
    ...
    ctx.beginPath();
    ctx.rect(paddle.x, paddle.y, paddle.width, paddleHeight);
...
function drawBricks() {
    ...
    // Calculate the total width of all bricks
    const totalBrickWidth = (brickWidth + brickPadding) * level.brickCols - brickPadding;
    const startX = (canvas.width - totalBrickWidth) / 2; // Center the bricks

    for (let c = 0; c < level.brickCols; c++) {
        for (let r = 0; r < level.brickRows; r++) {
            if (bricks[c][r].status === 1) {
                const brickX = startX + (c * (brickWidth + brickPadding)); // Adjusted for centering
                const brickY = brickOffsetTop + (r * (brickHeight + brickPadding)); // Keep the vertical offset
...
function draw() {
    ...
    if(ball.y + ball.dy < ballRadius) {
        ...
        if(ball.x > paddle.x && ball.x < paddle.x + paddle.width) {
            ...
            ball.dy = -ball.dy; // Reverse the ball's direction
        } else {
            gameOver = true;
...
function initLevel() {
    ...
    ball.x = canvas.width / 2; // Centered in the new canvas size
    ball.y = canvas.height - 30; // Positioned above the bottom
    ball.dx = level.ballSpeed * (Math.random() < 0.5 ? 1 : -1);
    ball.dy = -level.ballSpeed;
    
    paddle.width = level.paddleWidth;
    paddle.x = (canvas.width - paddle.width) / 2; // Centered in the new canvas size
    ...
    for(let c = 0; c < level.brickCols; c++) {
        bricks[c] = [];
        for(let r = 0; r < level.brickRows; r++) {
            bricks[c][r] = { x: 0, y: 0, status: 1 }; // Ensure status is set to 1 for all bricks
        }
    }
